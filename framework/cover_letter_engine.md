...
```mermaid
flowchart TD
    A[Start: Collect Inputs] --> B[Capture JD, Role, Company, Resume, Target Angle]
    B --> C[Decompose Job Description]

    C --> C1[Extract Responsibilities]
    C --> C2[Extract Qualifications and Tools]
    C --> C3[Extract Soft Skills and Culture Signals]
    C --> C4[Infer Hidden Business Needs]

    C1 --> D[Prioritize Requirements]
    C2 --> D
    C3 --> D
    C4 --> D

    D --> D1[Tier 1 Critical]
    D --> D2[Tier 2 Important]
    D --> D3[Tier 3 Supporting]

    D1 --> E[Match Candidate Experience]
    D2 --> E

    E --> E1[Write Mirror Statements]
    E1 --> E2[Write Proving Statements]
    E2 --> E3[Write Outcome Statements]

    E3 --> F[Detect Repeatable Strength Patterns]
    F --> G[Build Role-to-Expertise Bridge]
    G --> H[Select Identity Hook]

    H --> I[Choose 2 to 4 Relevant Proof Points]
    I --> J[Assemble One-Page Letter]

    J --> J1[Paragraph 1 Positioning Intro]
    J --> J2[Paragraph 2 Bridge Paragraph]
    J --> J3[Paragraph 3 Selected Proof]
    J --> J4[Paragraph 4 Close]

    J4 --> K[Run Quality Checks]

    K --> K1{Resume Rehash?}
    K --> K2{Strong Bridge?}
    K --> K3{Memorable Identity?}
    K --> K4{Enough Proof?}
    K --> K5{Addresses Tier 1 Needs?}
    K --> K6{Fits One Page?}

    K1 --> L{Pass All Checks?}
    K2 --> L
    K3 --> L
    K4 --> L
    K5 --> L
    ...
    L -- No --> M[Revise Positioning, Proof, and Structure]
    M --> J

    L -- Yes --> N[Generate Final Outputs]
    N --> N1[Full Cover Letter]
    N --> N2[Short Email Version]
    N --> N3[Application Text Box Version]
    N --> N4[Interview Alignment Talking Points]
```
...
flowchart LR
    ...
    subgraph Analysis
        B1[Extract Explicit Requirements]
        B2[Infer Hidden Needs]
        B3[Rank Critical vs Secondary]
    end

    subgraph Matching
        C1[Mirror Statement]
        C2[Proving Statement]
        C3[Outcome Statement]
    end

    subgraph Positioning
        D1[Identify Repeatable Strengths]
        D2[Build Bridge Statement]
        D3[Select Identity Hook]
    end
    ...
    subgraph Outputs
        G1[Cover Letter]
        G2[Email Version]
        G3[Application Version]
        G4[Interview Talking Points]
    end
